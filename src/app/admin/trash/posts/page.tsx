...
async function getProfileAndDeletedPosts(page: number) {
...
export default async function AdminDeletedPostsPage({ searchParams }: PageProps) {
  const { page: pageParam } = await searchParams;
  const page = Math.max(1, Number(pageParam) || 1);

  const { profile, posts, totalCount } = await getProfileAndDeletedPosts(page);
    ...
    <AdminLayout profile={profile}>
      <div className="p-6 lg:p-8">
          ...
          <CardContent>
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm text-zinc-500">
                총 {totalCount}개의 삭제된 게시글 · 페이지 {page}/{totalPages}
              </span>
            </div>

            {posts.length === 0 ? (
              <p className="text-center text-zinc-500 py-8">삭제된 게시글이 없습니다.</p>
            ) : (
              <Grid cols={1} gap="sm">
        ...
        <Stack direction="row" justify="center" gap="sm">
          <Link href={`/admin/trash/posts?page=${page - 1}`} aria-disabled={page <= 1}>
            <Button
              variant="outline"
              size="sm"
              disabled={page <= 1}
          ...
          <Link href={`/admin/trash/posts?page={page + 1}`} aria-disabled={page >= totalPages}>
            <Button
              variant="outline"
              size="sm"
