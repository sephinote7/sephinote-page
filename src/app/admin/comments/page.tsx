...
async function getProfileAndComments(page: number) {
...
export default async function AdminCommentsListPage({ searchParams }: PageProps) {
  const { page: pageParam } = await searchParams;
  const page = Math.max(1, Number(pageParam) || 1);

  const { profile, comments, totalCount } = await getProfileAndComments(page);
    ...
    <AdminLayout profile={profile}>
      <div className="p-6 lg:p-8">
          ...
          <CardContent>
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm text-zinc-500">
                총 {totalCount}개의 댓글 · 페이지 {page}/{totalPages}
              </span>
            </div>

            {comments.length === 0 ? (
              <p className="text-center text-zinc-500 py-8">등록된 댓글이 없습니다.</p>
            ) : (
              <Grid cols={1} gap="sm">
        ...
        <Stack direction="row" justify="center" gap="sm">
          <Link href={`/admin/comments?page=${page - 1}`} aria-disabled={page <= 1}>
            <Button
              variant="outline"
              size="sm"
              disabled={page <= 1}
          ...
          <Link href={`/admin/comments?page=${page + 1}`} aria-disabled={page >= totalPages}>
            <Button
              variant="outline"
              size="sm"
